...
module.exports = async function (creater, params, cb) {
  ...
  const pageDir = path.join(projectDir, pageSrc, pageName)
  ...
  try {
    ...
    if(pageName === 'pages'){
      creater.removePagePackages(pageSrc, subPageName)
        ...
        creater.removeSubpackages(pageSrc, subPageName, pageName, true)
    }else creater.removeSubpackages(pageSrc, subPageName, pageName, false)
